...
도구 사용 내역을 `.contexty/sessions/<sessionId>/tool-parts.json`에 기록하며, VSCode 확장과 연동하여 다음 기능을 제공합니다. 기존 루트 경로 `.contexty/tool-parts.json`는 읽기 호환용 legacy 포맷으로만 유지됩니다.
...
| **블랙리스트**        | 제거된 항목을 `.contexty/sessions/<sessionId>/tool-parts.blacklist.json`에 저장하여 영구 제외 |
